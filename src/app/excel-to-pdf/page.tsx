...
export default function ExcelToPdfPage() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      setProgress(55);
      const res = await fetch('/api/excel-to-pdf', { method: 'POST', body: fd });
      setProgress(85);
      if (!res.ok) { const e = await res.json(); throw new Error(e.error || 'Failed'); }
      const blob = await res.blob();
      const key = cacheDownload(blob, 'converted.pdf');
      setProgress(100);
      router.push(`/download?key=${key}&file=converted.pdf&tool=excel-to-pdf`);
    } catch (err: unknown) {
  ...
  return (
    <ToolPageLayout title="Excel to PDF" description="Convert Excel spreadsheets (.xlsx, .xls, .csv) to PDF. Formatting preserved."
      iconColor="bg-green-50 text-green-700" toolPath="/excel-to-pdf"
      icon={<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><path d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7"/></svg>}>

      {!file && <DropZone accept=".xlsx,.xls,.csv,.ods" multiple={false} label="Drop your file here" subLabel="Select your XLSX · XLS · CSV · ODS file" onFilesSelected={f => { setFile(f[0]); setStatus('idle'); }} />}
      ...
      <div className="mt-12 pt-8 border-t border-ink-100 seo-prose">
        ...
        <p>Upload your Excel file to convert it to a PDF with formatting and layout preserved. Column widths, cell formatting, charts, and formulas are rendered into a fixed PDF layout ready for sharing or printing.</p>
